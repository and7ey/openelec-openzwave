...
{
    string port = argv[1]; // "/dev/ttyUSB0"
    string onoff = argv[2]; // 1 or 0
    bool consoleoutput = (argv[3] == std::string("1"));

    //int nodeid = atoi(argv[2]); // 6
    //int classid = strtol(argv[3], NULL, 16); // 0x25
    //int offvalue = atoi(argv[4]); // 0
    //int onvalue = atoi(argv[5]); // 99
    //bool consoleoutput = (strtol(argv[6], NULL, 0) == 1);
    //printf("\n Command Line parameters: nodeid %i, classid %i, offvalue %i, onvalue %i, consoleoutput %i \n ====== \n", nodeid, classid, offvalue, onvalue, consoleoutput );
   ...
   // Set up mutual exclusion so that this thread has priority
    ...
    Options::Get()->AddOptionBool( "ConsoleOutput", consoleoutput );
    ...
    // switch on/off
    //printf("\n Node Id %i, Class Id %i \n ====== \n", nodeid, classid );
    //SetValue(50, nodeid, classid);
    //sleep(5);
    //SetValue(0, nodeid, classid);  
    //sleep(5);
    //printf("\n\nswitch all off");


    if (onoff == std::string("0"))
      {
        printf("\n Switch All Off \n");
	Manager::Get()->SwitchAllOff(g_homeId);
      ...
      {
        printf("\n Switch All On \n");
        //Manager::Get()->SetNodeOn(g_homeId, 6);
	Manager::Get()->SwitchAllOn(g_homeId);
	//Manager::Get()->SetNodeLevel(g_homeId, 6, 99);
      }    
